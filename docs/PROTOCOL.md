...
현재 앱은 키보드 설정 창을 열 때 각 프로필의 엔트리 0/1/2를 조회한다.
앱 실행이 지정된 Profile 2~5 슬롯은 첫 쓰기 전에 12개 원본 56바이트를 모두 읽어
설정 파일에 별도 백업하고, 해당 슬롯의 출력 데이터만 왼쪽 F16·가운데 F17·오른쪽
F18로 RAM 임시 매핑한다. 각 쓰기는 56바이트 전체 readback으로 검증하며 부분 실패 시
이미 변경한 키를 역순 원복하고 조회 전 활성 프로필을 복원한다. 정상 종료 또는 앱 실행
설정 해제 시 앱이 변경한 슬롯을 원본으로 복원한다. Profile 1의 엔트리 0~2는 읽거나
쓰지 않는다.

엔트리 15/14의 원본 56바이트는 먼저 읽어 설정 파일에 백업하고,
출력 타입/데이터만 각각 F20/F19로 바꾼다. 쓰기 후 같은 출력 동작인지 readback으로
검증하며, 설정 비활성화와 정상 종료 때 원래 Vol-/Vol+ 엔트리로 복원한다. 노브 클릭은
쓰지 않으며 `0x0D Save`도 보내지 않는다.
...
4. `0x10 KeyInfo`로 Profile 2~5 하단 키 12개를 백업하고 앱 실행 슬롯만 F16~F18로 RAM 임시 적용
5. `0x10 KeyInfo`로 노브 좌/우 원본을 백업하고 F20/F19를 RAM에 임시 적용
6. `0x25 Display`로 RGB565 프레임을 RAM 전송
7. 정상 종료 시 하단 앱 실행 키와 노브 원본을 복원하고, 분리 시 3초 간격으로 재탐색
...
현재 앱 코드에는 Save, LED, 플래시, 부트로더 명령 상수가 없다. KeyInfo 쓰기는 Profile
2~5 하단 엔트리 0~2의 앱 실행용 F16~F18 RAM 임시 매핑과 노브 좌/우 엔트리 15/14의
F20/F19 RAM 임시 매핑으로만 제한한다.
...
사용한다. 해석하지 못한 KeyInfo는 오류 코드 없이 `미지원`으로 표시한다. 앱 실행 설정을
저장하면 F16~F18 전역 단축키 등록 성공 후 Profile 2~5의 해당 하단 키를 RAM에 자동
적용한다. 12개 전체 사전 백업, 쓰기별 readback, 부분 실패 역순 원복, 활성 프로필 복원,
정상 종료 원복을 수행한다. 기존 노브 엔트리 15/14와 F20/F19 처리 순서는 유지한다.

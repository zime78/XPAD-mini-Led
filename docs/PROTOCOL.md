...
현재 앱 코드에는 Save, LED, 일반 키맵/설정, 플래시, 부트로더 명령 상수가 없다.
KeyInfo는 위 노브 좌/우 RAM 임시 매핑 범위에서만 사용한다.

...
전송한 뒤 설정을 다시 읽는다. 현재 앱의 키보드 설정 조회도 이 방식을 RAM 전환으로만
사용한다. 조회 전 활성 프로필을 기억하고 편집 대상인 P2~P5의 KeyInfo를 읽은 다음 원래
프로필로 복원하며, 각 전환과 최종 복원은 SystemInfo readback으로 검증한다. P1은 장치에서
읽지 않고 고정 음악 제어값을 사용한다. `0x0D Save`는 보내지 않는다.
...
2. `0x02 ScreenInfo`로 `240 × 135` 확인
3. `0x10 KeyInfo`로 노브 좌/우 원본을 백업하고 F20/F19를 RAM에 임시 적용
4. `0x25 Display`로 RGB565 프레임을 RAM 전송
5. 비활성화/정상 종료 시 노브 원본을 복원하고, 분리 시 3초 간격으로 재탐색
...
실제 KeyInfo를 읽고 조회 전 프로필로 복원한다. 제품 정책상 Profile 1은 장치 조회 대상에서도
제외하고 이전 곡·재생/일시정지·다음 곡으로 정규화해 보기 전용으로 고정한다. Profile 2~5만 로컬 편집·사용자 백업 대상으로
사용한다. 해석하지 못한 KeyInfo는 오류 코드 없이 `미지원`으로 표시한다. F16~F18 action
router는 로컬에서 관리한다. 일반 키 쓰기 전체 rollback은 아직 구현하지 않았으므로

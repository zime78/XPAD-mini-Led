...
표시한다. 패널 바깥 여백은 6px만 두어 카드 아래 빈 영역이 생기지 않는다.
...
장치를 전환하지 않는다. F16~F18 앱 실행 연결은 해당 슬롯의 실제 장치 키가 F16/F17/F18일
...
않는다. 사용자가 활성화하면 `F16`~`F18`만 전역 단축키로 등록하고 활성 프로파일의 세
동작으로 라우팅한다. 기존 미세 볼륨의 `F19`/`F20`은 등록·해제하지 않는다.
...
구현했다. Profile 1은 고정값을 사용해 장치에서 읽거나 쓰지 않는다. 일반 키 쓰기 전체
...
| 명령 | ID | 사용 목적 |
|---|---:|---|
| `ScreenInfo/SystemInfo` | `0x02` | LCD 확인, P1~P5 RAM 순회와 원래 프로필 복원 |
...
따라서 이번 실기기 적용은 **LCD RAM 화면 갱신, 조회 중 프로필 RAM 임시 전환·원복,
노브 좌/우 RAM 임시 매핑**이며 **펌웨어 업데이트가 아니다**.
...
| 대상 | 파일 | SHA-256 |
|---|---|---|
| Apple Silicon | `dist/XPAD Mini Now Playing-0.1.0-arm64.dmg` | `b2a381ffb94607e9ee45079dfbdb122f86531bb20d062d055b5108f4f8d84868` |
...
| 자동 테스트 | Vitest 9개 파일, 42개 테스트 통과 — 기존 UI·키보드 설정·백업·라우팅 검증에 더해 볼륨 조절 후 실제 readback 이벤트, OSD 값·비례 막대·0/100% 경계 클램프 검증 |

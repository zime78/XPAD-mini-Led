...
| `0xFF12` | `0x02` | Sayo API v2 고속, 1024바이트 | **LCD와 제한된 노브 RAM 매핑용** |
...
| 전체 설정 | 키맵, 성능, RGB, LCD, 펌웨어 등 | LCD RAM + 노브 좌/우 RAM 임시 매핑만 구현 |
...
| 프로필 | `0x02 SystemInfo.cfg_selection` + `0x10 KeyInfo` | **P1~P5 RAM 순회 읽기 확인** | 현재 앱은 P1을 고정값으로 보호하고 P2~P5만 읽은 뒤 원래 프로필을 readback 검증해 복원. Save 미사용 |
...
| 3 | `0x02`로 `240 × 135` 확인 | [`protocol.ts`](../src/main/device/protocol.ts) |
| 4 | `0x10`으로 노브 좌/우 원본 백업 후 F20/F19 임시 적용 | [`protocol.ts`](../src/main/device/protocol.ts) |
| 5 | Apple Music/Spotify 현재 곡 읽기 | [`now-playing.ts`](../src/main/music/now-playing.ts) |
| 6 | 화면 렌더링/RGB565 변환 | [`frame-renderer.ts`](../src/main/display/frame-renderer.ts) |
| 7 | `0x25`로 변경 청크 RAM 전송 | [`protocol.ts`](../src/main/device/protocol.ts) |
| 8 | 비활성화/정상 종료 시 노브 원복, 분리 시 3초마다 재연결 | [`device-worker.ts`](../src/main/device/device-worker.ts) |

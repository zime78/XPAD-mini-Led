...
전환하며 readback 성공 후에만 선택 표시와 세 키 요약을 갱신한다. F16~F18 앱 실행 연결은 해당 슬롯의 실제 장치 키가 F16/F17/F18일
때만 로컬 앱 경로를 합쳐 표시한다. 해석하지 못한 장치 동작은 내부 원시값을 화면에
...
않는다. 사용자가 활성화하면 `F16`~`F18`만 전역 단축키로 등록한다. 실제 장치 프로필 전환이
...
장치 쓰기가 발생하지 않으며, 사용자가 `로컬 설정 저장`을 눌러야 활성 설정이 바뀐다.

Profile 2~5 조회는 `0x02 SystemInfo`의 `cfg_selection` RAM 전환과 `0x10 KeyInfo` 읽기로
구현했다. Profile 1은 하단 KeyInfo를 읽거나 쓰지 않고 고정값을 표시하지만, 재생 화면의
실제 P1 선택은 다른 프로필과 같은 `0x02` RAM 전환을 사용한다. 일반 키 쓰기 전체
rollback은 아직 구현하지 않아 `장치에 적용`은 차단 사유와
함께 비활성화되어 있으며, 이번 구현은 일반 키 `KeyInfo` 쓰기, Save, MemoryWrite, LED,
부트로더 명령을 추가하지 않았다. 키보드 전용 저장
...
| 자동 테스트 | Vitest 9개 파일, 46개 테스트 통과 — P1~P5 UI 전환·잠금, `0x02` 프로필 선택·readback, 선택 프로필 F16~F18 라우팅과 기존 볼륨 OSD·키보드 설정 검증 |

...
- 상태: P1 고정 음악 제어, P2~P5 실제 장치 읽기·로컬 설정·백업·F16~F18 단축 동작,
  재생 화면 P1~P5 실제 전환, 앱 실행 슬롯의 F16~F18 RAM 임시 매핑·원복 구현 완료
- 작성일: 2026-07-22
...
- 적용 경계: P1~P5 `cfg_selection` 선택·원복과 P2~P5 하단 KeyInfo 조회를 바탕으로,
  앱 실행 슬롯의 F16~F18 RAM 임시 쓰기만 허용한다. 일반 키 선택값과 P1은 계속 쓰지 않는다.
...
| 하단 3버튼의 XPAD HID 키맵 변경 | 앱 실행 슬롯 완료 | P1은 쓰기 금지로 고정. P2~P5의 앱 실행 슬롯만 F16~F18로 RAM 임시 매핑하며 12개 사전 백업·readback·부분 실패 역순 원복·종료 복원 구현 |
| 볼륨·노브 비간섭 | 완료 | 키보드 전용 저장 IPC는 `fineVolume*`, `knobKeymapBackup`, 노브 장치 경로를 호출하지 않음 |

앱 실행 슬롯의 장치 `backup/apply/readback/rollback` 안전 게이트는 구현했다. P2~P5
하단 12개 원본을 첫 쓰기 전에 모두 확보하고 앱 실행 슬롯만 F16~F18로 바꾸며, P1과
노브는 건드리지 않는다. 일반 키 선택값의 장치 쓰기와 Save/플래시 명령은 전송하지 않는다.
...
### 단계 5 — 승인된 장치 worker/프로토콜 확장 (앱 실행 슬롯 완료)

- 앱 실행으로 지정한 하단 물리 버튼만 F16~F18로 read/backup/apply/readback/rollback/restore한다.

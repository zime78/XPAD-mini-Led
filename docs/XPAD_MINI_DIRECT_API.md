...
않는다. VID만으로 열지 않는다. `0x3710`은 다른 HID에도 쓰인 사례가 있어, 열기
조건은 위 네 값(VID / PID / bulk page / usage)을 모두 만족하는 컬렉션이다.
...
| 프로필 | `0x02 SystemInfo.cfg_selection` + `0x10 KeyInfo` | **P1~P5 RAM 선택·P2~P5 읽기/제한 쓰기 확인** | 재생 화면은 P1~P5를 직접 선택. 키보드 설정은 P1을 보호하고 P2~P5 하단 12개를 사전 백업한 뒤 앱 실행 슬롯만 F16~F18로 RAM 임시 매핑·readback·실패 원복. Save 미사용 |

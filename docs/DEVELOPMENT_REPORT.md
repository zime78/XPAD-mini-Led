...
| 일반 키 장치 적용 | 실제 조회는 지원하되 일반 키 쓰기 전체 rollback 미구현으로 UI에서 차단 | 보류 |
...
2. 장치 응답의 화면 크기 240×135와 실제 활성 프로필을 확인한다.
3. 검증에 성공해야 `protocolReady`를 활성화한다.
4. 재생 화면 요청은 같은 `0x02`로 P1~P5를 RAM에서 선택하고 SystemInfo readback을
   확인한 뒤 화면과 F16~F18 라우터를 갱신한다.
5. `KeyInfo(0x10)`로 노브 좌/우 원본을 백업하고 앱 전용 F20/F19를 RAM에 적용한다.
6. `Display(0x25)` 명령으로 RGB565 프레임을 최대 1,012바이트씩 나누어 전송한다.
7. 동일한 청크는 생략하고, 약 66초마다 전체 프레임을 다시 보내 동기화를 보강한다.
8. 화면에 변화가 없어도 250ms 이상 전송이 없으면 첫 청크를 다시 보내며, Worker의
   기본 스트리밍 주기는 220ms다.
...
조절이 성공하면 `FineVolumeController`가 macOS readback으로 확인한 실제 출력값을 main에
전달한다. main은 기존 곡 프레임 위에 큰 퍼센트 숫자와 막대를 합성해 LCD와 앱 미리보기에
동시에 표시하고, 마지막 조절 완료 1.6초 뒤 곡 화면을 복원한다. 연속 입력은 최신 값으로
교체하면서 타이머를 다시 시작하며 0%·100% 경계에서도 현재 값을 표시한다. 이 피드백은
...
앱을 열면 680×320 고정 재생 창에 별도 소개 헤더 없이 재생 패널만 표시한다. 패널 왼쪽 위에는 USB·LCD
프로토콜·XPAD 노브 상태를 텍스트 없이 장치 아이콘과 녹색 점(연결) 또는 빨간 ×(실패)로
표시한다. 상태 아이콘 오른쪽에는 별도 제목 없이 P1~P5 버튼과 선택 프로필의 하단 세 키
요약을 표시한다. 사용 중 버튼과 세 키 값은 파란색으로 구분한다. 패널 바깥 여백은 6px만
두어 카드 아래 빈 영역이 생기지 않는다.

패널 오른쪽 위 설정 아이콘을 누르면 재생 창은 그대로 유지되고 760×690 독립 설정 창이
열린다. 이미 열린 설정 창은 새로 만들지 않고 앞으로 가져온다. 닫기 아이콘은 설정 창만
닫는다. main 프로세스가 두 `BrowserWindow`의 생성·재사용과 상태 브로드캐스트를 담당하고,
preload가 창 열기/닫기 IPC를 안전하게 노출한다. 공용 renderer는 URL의 `view` query로 창
역할을 구분하며 재생 화면, 소형 상태 표시, 상세 장치 상태, 표시 설정, 노브 설정을 각각
독립 컴포넌트로 구성한다.
...
중복 실행하지 않고 재생 창을 앞으로 가져온다.

### 4.6 키보드 설정, 앱 실행과 사용자 백업

재생 창의 설정 아이콘 왼쪽에 키보드 아이콘을 추가했다. 아이콘을 누르면 1,080×760
독립 `BrowserWindow`가 열리며, 다시 누르면 기존 키보드 창을 재사용하고 포커스한다.
창은 `Profile 1`~`Profile 5`와 각 프로파일의 하단 왼쪽·가운데·오른쪽 버튼만 표시한다.
볼륨·노브·PF1·화면·원형 컨트롤은 편집 대상에 포함하지 않는다.

키보드 창이 열리면 SystemInfo의 `cfg_selection`을 RAM에서 P2~P5로 순회하며 각 프로필의
하단 KeyInfo 엔트리 0~2를 읽는다. 조회 전 활성 프로필은 마지막에 복원하고 SystemInfo
readback으로 확인한다. Profile 1은 장치 조회에서 제외하고 이전 곡·재생/일시정지·다음 곡으로 정규화해 보기
전용으로 고정하고, Profile 2~5만 사용자 편집 대상으로 사용한다. 프로필 탭 클릭 자체는
장치를 전환하지 않는다. 반면 재생 화면의 P1~P5 단축 버튼은 실제 `cfg_selection`을 RAM에서
전환하며 readback 성공 후에만 선택 표시와 세 키 요약을 갱신한다. 앱 실행 설정을 저장하면
P2~P5의 해당 슬롯을 왼쪽 F16·가운데 F17·오른쪽 F18로 RAM 임시 매핑하고 로컬 앱 경로를
합쳐 표시한다. 앱 실행 연결은 로컬 논리 설정을 표시 기준으로 사용하므로 장치 재조회가
해당 슬롯을 미지원으로 반환해도 설정 창과 재생 창에서 저장된 앱 이름을 유지한다.
해석하지 못한 장치의 비앱 동작은 내부 원시값을 화면에
노출하지 않고 한 줄 `미지원`으로 표시한다.
XPAD Mini USB 연결 또는 LCD 프로토콜 준비가 실패하면 P1~P5 탭, P2~P5 키 편집, 앱 선택·테스트,
로컬 저장과 사용자 백업 전체를 비활성화한다. 같은 조건에서 일반 설정의 표시·노브 설정과
저장·되돌리기도 비활성화하며, renderer 방어를 우회한 저장 요청은 main IPC가 다시 거부한다.
각 논리 버튼에는 영문·숫자·기호·기본/탐색/기능/미디어 키 또는 native dialog에서 고른
macOS `.app` 실행을 설정할 수 있다. 미디어 키는 이전 곡, 재생/일시정지, 다음 곡을
`키 변경` 목록에 포함한다. 앱 실행은 main process에서 절대경로와 `.app` 디렉터리를
검증한 뒤 `shell.openPath()`만 사용한다. 셸 명령, 인자, URL, 임의 AppleScript는 받지
않는다. 앱 실행 설정이 하나라도 있으면 `F16`~`F18` 전역 단축키를 자동 활성화하고, 세
단축키 등록이 모두 성공한 뒤에만 장치 RAM 매핑을 시작한다. 실제 장치 프로필 전환이
성공하면 같은 프로필을 로컬 라우터에도 선택해 세 단축키가 해당 프로필의 동작으로 매핑된다.
프로필이 바뀌어도 F16~F18 전역 단축키를 중복 등록하지 않는다. 기존 미세 볼륨의
`F19`/`F20`은 등록·해제하지 않는다.

이름 1~40자와 설명 0~500자를 갖는 사용자 백업은
`userData/keyboard-backups.json`에 최대 10개 저장한다. 백업은 활성 프로파일과 사용자
편집 대상인 Profile 2~5의 논리 설정 12개를 깊은 복사한다. v1 파일 호환을 위해 Profile 1
필드도 저장하지만 생성·읽기·복원 때마다 고정 음악 제어값으로 정규화한다. 파일은 임시 파일
작성 후 rename으로 교체한다.
손상 경고가 있는 파일은 새 데이터로 덮어쓰지 않는다. 복원은 편집 초안만 바꾸므로
장치 쓰기가 발생하지 않으며, 사용자가 `저장하고 장치에 적용`을 눌러야 활성 설정이 바뀐다.

Profile 2~5 앱 실행 매핑은 `0x02 SystemInfo`의 `cfg_selection` RAM 전환과 `0x10 KeyInfo`
읽기·쓰기로 구현했다. 첫 쓰기 전에 하단 12개 원본을 모두 백업하고, 쓰기마다 56바이트
전체 readback을 확인한다. 부분 실패 시 변경 키를 역순 원복하고 원래 활성 프로필로
돌아간다. 정상 종료와 앱 실행 설정 해제 때 앱이 변경한 키를 복원한다. Profile 1은 하단
KeyInfo를 읽거나 쓰지 않고 고정값을 표시하지만, 재생 화면의 실제 P1 선택은 다른 프로필과
같은 `0x02` RAM 전환을 사용한다. Save, MemoryWrite, LED, 부트로더 명령은 추가하지 않았다. 키보드 전용 저장
IPC는 일반 설정 저장과 분리하고, 일반 설정 창도 현재 키보드 설정을 보존해 두 창의 저장
순서가 서로의 값을 덮어쓰지 않게 했다.
...
| 명령 | ID | 사용 목적 |
|---|---:|---|
| `ScreenInfo/SystemInfo` | `0x02` | LCD·활성 프로필 확인, P1~P5 직접 RAM 선택, 설정 조회 순회와 원래 프로필 복원 |
| `KeyInfo` | `0x10` | P2~P5 하단 앱 실행 키의 F16~F18 RAM 임시 매핑/복원, 노브 좌/우 원본 읽기와 RAM 임시 매핑/복원 |
| `Display` | `0x25` | 장치 RAM의 LCD 프레임 갱신 |

현재 코드에는 아래 동작이 포함되어 있지 않다.

- `Save(0x0D)` 및 플래시 영구 저장
- Profile 1 하단 키, P2~P5의 앱 실행 슬롯 외 키맵과 노브 클릭 쓰기
- 키 입력 전송 또는 매크로 실행
- LED 설정 변경
- 펌웨어 업데이트 또는 부트로더 진입
- 공장 초기화

따라서 이번 실기기 적용은 **LCD RAM 화면 갱신, 사용자 선택 프로필 RAM 전환, 조회 중
프로필 RAM 임시 전환·원복, 노브 좌/우 RAM 임시 매핑**이며 **펌웨어 업데이트가 아니다**.
...
2026-07-22에 다음 항목을 다시 확인했다.

| 검증 | 결과와 근거 |
|---|---|
| TypeScript | `npm run typecheck` 통과 |
| 자동 테스트 | Vitest 9개 파일, 52개 테스트 통과 — P1~P5 UI 전환·잠금, P2~P5 하단 12개 사전 백업, 앱 슬롯 F16~F18 적용·종료 복원·장치 재조회 시 논리 설정 유지, 부분 실패 역순 원복, 선택 프로필 라우팅과 기존 볼륨 OSD 검증 |
| 프로덕션 빌드 | `./build.sh check` 통과 |
| 키보드 dev UI | `./build.sh dev-ui`에서 키보드 아이콘, 1,080×760 독립 창, P1 고정/P2~P5 탭, 하단 3버튼, 백업 0/10을 확인하고 HID 비활성 상태에서 모든 키보드 설정 컨트롤이 잠기는지 확인 |
| 일반 설정 dev UI | HID 비활성 상태에서 연결 필요 안내와 표시·노브·저장·되돌리기 컨트롤 잠금 확인 |
...
| 앱 실행 키 RAM 매핑 | Fake HID에서 첫 쓰기 전 P2~P5 하단 12개를 모두 읽고 P1은 접근하지 않으며, 중간 실패 시 변경 키와 원래 활성 프로필이 복원되는지 확인. 서명 앱에서 P2 왼쪽이 장치 F16 readback과 병합되어 `Discord`로 표시되는 것을 확인했고, 사용자가 물리 키를 눌러 Discord 실행이 유지됨을 확인. 장치 재조회가 미지원 값을 반환하는 회귀 조건에서도 로컬 앱 연결을 유지하는 단위 테스트 통과 |
...
5. LCD 명령은 비공식 역공학 프로토콜에 근거한다. 현재 앱은 실기기로 검증한 최소 세

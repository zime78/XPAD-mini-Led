...
3. **오케스트레이션.** `main/index.ts`: single-instance lock, 트레이, 설정 창, IPC(`get-status`/`get-config`/`set-config`/`refresh-now-playing`, push `status-changed` — preload가 `window.xpad`로 노출). 렌더 큐는 시퀀스 번호로 최신 요청만 장치에 반영한다(stale 렌더 드롭). 종료 시 monitor 정지 후 노브 원본 복원을 포함한 worker shutdown을 최대 4초 기다린다.
